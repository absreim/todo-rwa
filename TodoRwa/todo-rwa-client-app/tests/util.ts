...
export const startWebApi = (env: string, urls: string, dbConnStr: string) => {
  return exec("npm run web-api", { env: { NODE_ENV: "test", ASPNETCORE_ENVIRONMENT: env, ASPNETCORE_URLS: urls, "Postgres:ConnectionString": dbConnStr }});
};

export const confirmWebApiRunning = async (url: string, retryIntervalMs: number = 2000, retryAttempts: number = 5) => {
  console.log(url)
