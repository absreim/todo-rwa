...
test("Deleting items works", async ({ page }) => {
  for (let i = 0; i < seedData.length; i++) {
    await deleteRow(seedData[i], page);
    await expect(page.getByRole("grid")).toHaveAttribute(
      "aria-rowcount",
      String(seedData.length - i),
    );
  }
});
...
async function validateGridRow(todoItem: TodoItem, gridRow: Locator) {
  ...
  await expect(
    gridRow.locator('div[data-field="isComplete"]'),
...
async function addRow(todoItem: TodoItem, page: Page) {
  ...
  const dataRowContainer = page.getByRole("rowgroup");
  const addedRow = dataRowContainer
    .getByRole("row")
    .filter({
      has: page.getByRole("gridcell", {
        name: String(todoItem.id),
        exact: true,
      }),
    });
...
async function deleteRow(todoItem: TodoItem, page: Page) {
  const rowToEdit = page
    .getByRole("rowgroup")
    .getByRole("row")
    .filter({
  ...
  await rowToEdit.getByRole("menuitem", { name: "Delete" }).click();
}
